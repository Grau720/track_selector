...
// 🛑 SOLUCIÓN DEFINITIVA (Función correcta)
app.get('/generos', (req, res) => apiWrapper(req, res, async () => {
    // La función correcta es getAvailableGenreSeeds()
    const data = await spotifyApi.getAvailableGenreSeeds(); 
    
    // Acceso robusto a la respuesta:
    const genres = data.body?.genres || data.genres || [];
    
    if (genres.length === 0) {
        console.warn('⚠️ Spotify devolvió una lista de géneros vacía. (Verifica tu conexión si esto no es esperado)');
    }
