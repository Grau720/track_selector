...
// 🛑 ENDPOINT /generos CORREGIDO CON AISLAMIENTO TOTAL
app.get('/generos', async (req, res) => {
    // 🛑 SOLUCIÓN DE AISLAMIENTO: Creamos una nueva instancia del SDK para llamadas genéricas.
    // Esto previene conflictos de estado con el token de usuario que causa el 404.
    ...
    try {
        // 1. Obtener un token de cliente (App Token)
        const clientData = await isolatedApi.clientCredentialsGrant();
        const clientToken = clientData.body['access_token'];

        // 2. Configuramos el token de cliente en la instancia aislada
        isolatedApi.setAccessToken(clientToken);

        // 3. Hacemos la llamada al endpoint de géneros con el SDK limpio
        ...
        console.error('Error al cargar géneros (SOLUCIÓN DE AISLAMIENTO):', e);
        
        // Si sigue fallando con 404/401, el problema es la configuración del ClientID/Secret.
        res.status(e.statusCode || 500).json({ 
            error: `Fallo crítico al obtener géneros (${e.statusCode}). Por favor, verifica tus ClientID/Secret.`
        });
    }
});
