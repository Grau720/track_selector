...
// 🛑 ENDPOINT /generos (Forzando URL Base y Debug)
app.get('/generos', async (req, res) => {
    // 1. Instancia aislada, FORZANDO la URL base de la API
    const isolatedApi = new SpotifyWebApi({
        clientId: clientId,
        clientSecret: clientSecret,
        apiUrl: 'https://api.spotify.com/v1' // <--- FUERZA la URL base
    });

    try {
        // 2. Obtener un token de cliente (App Token)
        ...
        console.log('DEBUG: ✅ Client Token obtenido. Expiración:', clientData.body['expires_in'], 'segundos.');

        // 3. Configuramos el token de cliente
        isolatedApi.setAccessToken(clientToken);

        // 4. Hacemos la llamada al endpoint de géneros (FUNCIÓN CORRECTA)
        console.log('DEBUG: Intentando obtener la lista de géneros...');
        // URL esperada: https://api.spotify.com/v1/recommendations/available-genre-seeds
        const data = await isolatedApi.getAvailableGenreSeeds(); 
        
        const genres = data.body?.genres || data.genres || []; 
        
        console.log(`DEBUG: ✅ Géneros obtenidos. Total: ${genres.length}`);
        ...
        console.error('----------------------------------------------------');
        console.error(`❌ FALLO CRÍTICO en /generos. Status: ${statusCode}`);
        console.error(`❌ Mensaje: ${e.message}`);
        
        if (e.body) {
             console.error('❌ Cuerpo de la Respuesta de Error:', JSON.stringify(e.body, null, 2));
        }
        console.error('----------------------------------------------------');
        
        // Si el fallo persiste, la causa es 100% la librería o la red.
        res.status(statusCode).json({ 
            error: `Fallo al obtener géneros (${statusCode}). Causa probable: error interno de la librería/red.`,
            detail: e.message 
        });
    }
});
