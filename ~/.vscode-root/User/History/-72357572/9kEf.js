...
app.get('/generos', async (req, res) => {
    let tokenUsed = null;
    let originalToken = spotifyApi.getAccessToken(); // Guardamos el token de usuario

    try {
        // Opción 1: Intentar con el Token de Usuario (si existe)
        tokenUsed = originalToken;
        if (!tokenUsed) {
             // Opción 2: Usar o generar un Token de Cliente
             tokenUsed = await getClientToken();
        }
        
        // Configuramos el token temporalmente (puede ser de usuario o de cliente)
        spotifyApi.setAccessToken(tokenUsed);

        const data = await spotifyApi.getAvailableGenreSeeds();
        
        // Restauramos el token de usuario original ANTES de enviar la respuesta
        ...
        // Si hay un error, restauramos el token original
        if (originalToken) {
           spotifyApi.setAccessToken(originalToken);
        }
        
        console.error('Error al cargar géneros (FINAL):', e);

        // Enviamos la respuesta de error al frontend
        res.status(e.statusCode || 500).json({ 
            error: e.message || 'Fallo al obtener la lista de géneros disponibles de Spotify.'
        });
    }
});
