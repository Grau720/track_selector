...
app.get('/generos', async (req, res) => {
    try {
      // 1️⃣ Crear una instancia temporal solo para client credentials
      const isolatedApi = new SpotifyWebApi({
        clientId,
        clientSecret,
      });
  
      const data = await isolatedApi.clientCredentialsGrant();
      const clientToken = data.body['access_token'];
  
      console.log('✅ Token de cliente obtenido:', clientToken.substring(0, 20) + '...');
  
      // 2️⃣ Llamar directamente al endpoint oficial de Spotify
      ...
      if (!response.ok) {
        ...
        throw new Error(`Spotify devolvió ${response.status}: ${text}`);
      }
  
      const json = await response.json();
      const genres = json.genres || [];
  
      console.log(`✅ ${genres.length} géneros recibidos.`);
      res.json(genres);
    } catch (error) {
      console.error('❌ Error en /generos:', error.message);
      res
        .status(500)
        .json({ error: 'No se pudieron obtener los géneros.', detail: error.message });
    }
  });
