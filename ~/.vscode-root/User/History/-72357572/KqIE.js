...
// index.js - REEMPLAZAR el ENDPOINT /generos EXISTENTE
// 🛑 ENDPOINT /generos: BYPASS de la librería problemática usando 'fetch'
app.get('/generos', async (req, res) => {
    // 1. Instancia aislada para obtener el Client Token
    const isolatedApi = new SpotifyWebApi({
        clientId: clientId,
        clientSecret: clientSecret,
    });

    try {
        // 2. Obtener Client Token (ESTO SIEMPRE FUNCIONA)
        console.log('DEBUG: Intentando obtener Client Token...');
        const clientData = await isolatedApi.clientCredentialsGrant();
        const clientToken = clientData.body['access_token'];
        
        console.log('DEBUG: ✅ Client Token obtenido. Intentando obtener géneros con FETCH...');

        // 3. HACER LA LLAMADA DIRECTA CON FETCH (BYPASS)
        const apiResponse = await fetch('https://api.spotify.com/v1/recommendations/available-genre-seeds', {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${clientToken}`,
                // Añadimos un User-Agent para mayor robustez
                'User-Agent': 'TrackSelectionApp/1.0', 
            },
        });
        
        // 4. Manejo de Respuesta HTTP (captura el 404/500)
        if (!apiResponse.ok) {
            // Lee el cuerpo para intentar obtener un mensaje de error detallado
            const errorBody = await apiResponse.text();
            
            console.error('❌ FALLO EN FETCH. Status:', apiResponse.status, 'Body:', errorBody.substring(0, 100)); // Muestra los primeros 100 caracteres
            
            throw new Error(`API Error: ${apiResponse.status} ${apiResponse.statusText}. Respuesta: ${errorBody.substring(0, 50)}...`);
        }
        ...
        console.log(`DEBUG: ✅ Géneros obtenidos (vía fetch). Total: ${genres.length}`);

        res.json(genres);

    } catch (e) {
        // 6. Manejo de Errores detallado
        const statusCode = (e.message && e.message.includes('API Error:')) ? parseInt(e.message.split(' ')[2]) : 500;
        
        console.error('----------------------------------------------------');
        console.error(`❌ FALLO IRREVERSIBLE en /generos (vía fetch). Status: ${statusCode}`);
        console.error(`❌ Mensaje: ${e.message}`);
        console.error('----------------------------------------------------');
        
        res.status(statusCode).json({ 
            error: `Fallo al obtener géneros (${statusCode}). Causa probable: Red/Firewall o Bug de bajo nivel.`,
            detail: e.message 
        });
    }
});
