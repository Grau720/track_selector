...
app.get('/', async (req, res) => {
    ...
    // 💡 REFUERZO: Intentamos usar un endpoint ligero que requiere un token de usuario
    // para forzar el refresh si el token está a punto de expirar.
    spotifyApi.setAccessToken(token);
    try {
...
app.get('/generos', (req, res) => apiWrapper(req, res, async () => {
    // 💡 SOLUCIÓN: Usamos apiWrapper para que gestione la validación y el refresh del token de usuario.
    
    const data = await spotifyApi.getAvailableGenreSeeds();
    
    // Acceso a la respuesta corregido previamente
    const genres = data.body?.genres || data.genres || []; 
    
    if (genres.length === 0) {
        console.warn('⚠️ Spotify devolvió una lista de géneros vacía. (Esto es normal si no hay problema)');
    }
