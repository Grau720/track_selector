...
app.get('/generos', async (req, res) => {
    // Siempre usamos el token de cliente para esta llamada genérica.
    let tempUserToken = spotifyApi.getAccessToken();

    try {
        const clientToken = await getClientToken();
        
        // Configuramos temporalmente SOLO el token de cliente para esta llamada
        spotifyApi.setAccessToken(clientToken);

        const data = await spotifyApi.getAvailableGenreSeeds();
        
        // Restauramos el token de usuario original ANTES de continuar
        spotifyApi.setAccessToken(tempUserToken);

        const genres = data.body?.genres || data.genres || []; 
        res.json(genres);
        
    } catch (e) {
        // Restauramos el token original en caso de error
        spotifyApi.setAccessToken(tempUserToken); 

        console.error('Error al cargar géneros (FINAL Y AISLADO):', e);
        
        // Si el token de cliente falla, el servidor está mal configurado.
        res.status(e.statusCode || 500).json({ 
            error: e.message || 'Fallo de configuración: No se pudo obtener el token de cliente para géneros.'
        });
    }
});
