...
// 🛑 SOLUCIÓN FINAL: USANDO FETCH (BYPASS DE LIBRERÍA)
app.get('/generos', async (req, res) => {
    ...
    try {
        // 1. Obtener y configurar el token (ESTO FUNCIONA)
        ...
        // 2. HACER LA LLAMADA DIRECTA CON FETCH (BYPASS)
        const apiResponse = await fetch('https://api.spotify.com/v1/recommendations/available-genre-seeds', {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${clientToken}`,
                // Spotify a veces necesita un User-Agent
                'User-Agent': 'TrackSelectionApp/1.0', 
            },
        });
        
        if (apiResponse.status === 404) {
             throw new Error('404 NOT FOUND - Fallo de red/URL directo.');
        }
        ...
        // Manejo de Errores
        console.error('----------------------------------------------------');
        console.error(`❌ FALLO IRREVERSIBLE en /generos (vía fetch): ${e.message}`);
        console.error('----------------------------------------------------');
        
        res.status(500).json({ 
            error: `Fallo al obtener géneros. La única solución fue usar fetch, pero falló. Causa: Network/Firewall.`,
            detail: e.message 
        });
    }
});
