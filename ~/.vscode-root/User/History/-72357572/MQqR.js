...
// 🛑 ENDPOINT /generos (Versión Final de Aislamiento, LÍMPIDA)
app.get('/generos', async (req, res) => {
    // 1. Instancia aislada SIN forzar configuraciones
    ...
    try {
        ...
        isolatedApi.setAccessToken(clientData.body['access_token']);
        console.log('DEBUG: ✅ Client Token obtenido.');
        console.log(clientData.body['access_token'])

        // 3. Obtener la lista de géneros (FUNCIÓN CORRECTA)
        ...
        console.error('----------------------------------------------------');
        console.error(`❌ FALLO CRÍTICO FINAL en /generos. Status: ${statusCode}`);
        console.error(`❌ Mensaje: ${e.message}`);
        console.error('----------------------------------------------------');
        
        res.status(statusCode).json({ 
            error: `Fallo al obtener géneros (${statusCode}). Se ha verificado el código; la causa es externa.`,
        });
    }
});
