let config = {
    ...
    excludedArtists: [],
    playlistId: null,
    favoriteArtists: [] // 🛑 ACTUALIZADO: Nueva propiedad
  };
  let allGenres = [];
  let filteredArtists = [];
  let allPlaylists = [];
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
      ...
      if (!config.favoriteArtists) config.favoriteArtists = []; // Aseguramos que exista
  ...
  function getCookie(name) {
    // 1. Prepara el string de cookies para la búsqueda
    const nameEQ = name + "=";
    // 2. Divide el string de todas las cookies por el delimitador '; '
    const ca = document.cookie.split(';');

    // 3. Itera sobre cada parte para encontrar la cookie deseada
    for(let i = 0; i < ca.length; i++) {
        let c = ca[i];
        while (c.charAt(0) === ' ') c = c.substring(1, c.length); // Elimina espacios al inicio
        // 4. Si la cookie actual comienza con el nombre buscado, devuelve su valor
...
// ----------------------------------------
// --- FUNCIONES DE ARTISTAS FAVORITOS (NUEVAS) ---
// ----------------------------------------
...
// ----------------------------------------
// --- FUNCIONES DE HISTORIAL (MONITOR) (ACTUALIZADO) ---
// ----------------------------------------
    ...
    try {
      const res = await fetch('/config', {
        ...
        body: JSON.stringify({
            ...
            favoriteArtists: config.favoriteArtists // 🛑 Incluir favoritos
        })
      });
  ...
  async function updateFilteredArtists() {
    ...
    try {
      const artistasRes = await fetch('/artistas-filtrados', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ generos: config.generos || [] }) // Se sigue filtrando por géneros del seguido. Los favoritos se añaden en el servidor.
      });
