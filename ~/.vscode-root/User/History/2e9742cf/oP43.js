let config = {
  ...
  let filteredArtists = [];
  let allPlaylists = [];
  let userId = ''; // Para mostrar en la pestaña de conectores
  ...
  // --- FUNCIONES DE RENDERIZADO DE CONFIGURACIÓN (Generos/Artistas/Playlists) ---
      ...
      try {
          ...
          history.forEach(item => {
              ...
              li.addEventListener('click', (e) => {
                  // Evita que el click en la tabla colapse el detalle
                  if (e.target.tagName === 'TD' || e.target.tagName === 'TH' || e.target.tagName === 'TABLE') return;
          ...
          list.innerHTML = `<li class="empty-message" style="color: #e22134;">Error al cargar historial: ${err.message}</li>`;
      }
  }
  
  
  // --- FUNCIONES DE LÓGICA Y DATOS (Siguen casi igual, solo se actualiza el historial después de añadir) ---
  ...
  async function filtrarYAnadir() {
    ...
    try {
      ...
      // Si la ejecución fue exitosa, actualizamos la vista del Monitor
      await loadAndRenderHistory(); 
